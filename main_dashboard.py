...
# Helper function for memory-efficient slice rendering
def get_slice(data_slice):
    # Normalize for display
    d_min, d_max = np.min(data_slice), np.max(data_slice)
    if d_max > d_min:
        normalized = (data_slice - d_min) / (d_max - d_min)
    else:
        normalized = data_slice
...
with col1:
    ...
    if uploaded_file:
        ...
        try:
            ...
            # 3. Optimized Triple View (Memory Efficient)
            ...
            with v1:
                st.image(get_slice(data[s_idx, :, :]), caption="Sagittal", use_container_width=True)
            with v2:
                st.image(get_slice(data[:, c_idx, :]), caption="Coronal", use_container_width=True)
            with v3:
                st.image(get_slice(data[:, :, a_idx]), caption="Axial", use_container_width=True)
                
        except Exception as e:
            st.error(f"File process panradhla error bro: {e}")
    else:
    ...
    if uploaded_file:
        if st.button("🚀 Run PEP Inference"):
            with st.spinner("Eliminating Probabilities... Running 5 Binary Models..."):
                try:
                    # Run PEP Logic
                    prediction, scores = engine.predict(temp_path)
                    
                    st.metric(label="Final Diagnosis Prediction", value=prediction)
                    
                    # Probability Graph (Plotly)
                    ...
                    st.error(f"Inference run aagala: {e}")
    else:
        st.write("Results will appear here after inference.")

# Footer
st.divider()
st.caption("Developed by Lekha Sree J | Probability Elimination Pipeline (PEP)")
